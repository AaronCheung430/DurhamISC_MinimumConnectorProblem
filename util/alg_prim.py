...
def prim(graph):
    ...
    while weight_list:  # weight_list is not empty
        no_edges += 1   # add 1 to variable for counting how many edges are there in this graph
    ...
    return {"Algorithm": "Prim's Algorithm (without queue)", "Number of Nodes": len(seen_edges), "Numbers of Edges": no_edges, "Computation Time": p_f_time, "Path Found": mst, "Minimal Weight": total_weight}
...
def prim_queue(graph):
    ...
    # return a dict with algorithm info
    return {"Algorithm": "Prim's Algorithm (with queue)", "Number of Nodes": len(seen_edges), "Numbers of Edges": no_edges, "Computation Time": p_queue_f_time, "Path Found": mst, "Minimal Weight": total_weight}
